...
const Bucket: FC<BucketProps> = ({ queue, answerNumber }) => {
    const finalQueue = [...queue, ...Array(maxGuessesAllowed - queue.length)].reverse();
        ...
        <div className="w-1/3 flex flex-col items-center justify-center text-center border-2 rounded-md overflow-hidden">
            {finalQueue.map((value = "", index) => (
                ...
                >
                    ...
                    {Number(value) !== answerNumber && (
                        <i
                            className={`fas ml-4 ${Number(value) - answerNumber < 0 ? "fa-arrow-up" : "fa-arrow-down"}`}
                        ></i>
                    )}
                </div>
            ))}
...
const GuessTheNumber: FC = () => {
    ...
    const answerNumber = useMemo(() => Math.floor(Math.random() * 101), [resetKey]);
    ...
    const guessNumber = () => {
        if (queue.length === maxGuessesAllowed - 1 || Number(number) === answerNumber) {
    ...
    return (
        <div className="flex flex-col items-center justify-center mt-16">
            <div className="mb-4 font-semibold text-xl px-6 text-center">Guess a number between 0 to 100</div>
            <Input value={number} onChange={handleNumberChange} type="number" />
            <div className="my-4">
                <Button onClick={guessNumber} text="Guess" className="px-4 mx-2" disabled={disableGuess} />
